...
class ExecutionEngine:
    ...
    def _render_dashboard(self, spot: float, atm: int):
        ...
        if not self.positions:
            ...
            hdr = f"  {'LEG':<10} {VS} {'STRIKE':>7} {VS} {'SIDE':<5} {VS} {'ENTRY':>7} {VS} {'LTP':>7} {VS} {'ENTRY SPOT':>10} {VS} {'SPOT SL':>10} {VS} {'DEBOUNCE':>8} {VS} {'PNL':>11}  "
            ...
            for leg, pos in self.positions.items():
                ...
                if sl_state and is_short:
                    ...
                    debounce_str = f"{sl_state.get('breach_count', 0)}/2"
                else:
                    entry_spot_str = "—"
                    spot_sl_str = "—"
                    debounce_str = "—"
                ...
                row = (f"  {c_white}{leg:<10}{res} {VS} {c_white}{pos['strike']:>7}{res} {VS} {side_col}{pos['side']:<5}{res} {VS} "
                       ...
                       f"{c_white}{debounce_str:>8}{res} {VS} {pnl_col}{sign}₹{pnl:>9,.2f}{res}  ")
...
def prompt_user_variables():
    ...
    print(f"{Fore.YELLOW}Press [Enter] on any parameter to accept default value in brackets.{Style.RESET_ALL}\n")
    ...
    try:
        ...
        print(f"  ✅ Parameters Confirmed! Deploying V2 Pro with active settings...")
        print(f"{'═'*78}{Style.RESET_ALL}\n")
