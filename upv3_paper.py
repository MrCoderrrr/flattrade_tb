...
class NSEATMStreamer:
    ...
    def get_live_quote(self, strike: int, option_type: str) -> Dict[str, Any]:
                        ...
                        if val is not None:
                            try:
                                ...
                                if v_flt > 0:
                                    ...
                                    self.is_flattrade_live = True
                                    return {
                                        "lp": float(quote.get("lp", quote.get("ltp", v_flt)) or v_flt),
                                        "bid": float(quote.get("bp1", 0.0) or 0.0),
                                        "ask": float(quote.get("sp1", 0.0) or 0.0),
                                        "tsym": tsym,
                                        "ls": ls,
                                    }
        ...
        return {"lp": lp, "bid": 0.0, "ask": 0.0, "tsym": tsym, "ls": ls}
...
EXPIRY_EXIT_HOUR          = 14
EXPIRY_EXIT_MINUTE        = 30     # close short legs before the final expiry volatility window
...
class ExecutionEngine:
    ...
    def _get_exit_premium(self, strike: int, option_type: str) -> float:
        """Return the conservative executable buyback price for a short leg.

        LTP can understate the price at which a short option can actually be
        closed. Prefer the live ask, then fall back to LTP when a quote book is
        unavailable. No synthetic price is created.
        """
        q = self.market_data.streamer.get_live_quote(strike, option_type)
        try:
            ...
            if ask > 0:
                return ask
            return float(q.get("lp", q.get("ltp", 0.0)) or 0.0)
    ...
    def _enter_leg(self, leg: str, strike: int, side: str, spot: float, atr: float, dte_days: float = 2.0) -> bool:
        if side == "SELL" and dte_days <= EXPIRY_ENTRY_CUTOFF_DTE:
            log_warn(
                f"Skipping {leg} entry: expiry protection active at {dte_days:.2f} DTE. "
                "Short weekly options are not opened in the expiry window."
            )
            return False
    ...
    def run(self):
        ...
        while self.is_running and not _EMERGENCY_STOP_TRIGGERED:
            try:
                ...
                try:
                    _, session_dte_days = self.market_data.streamer.get_near_expiry_dte()
                except Exception:
                    session_dte_days = 999.0
                ...
                # Check normal square-off and the earlier expiry-day safety
                # exit. Do not rely on theta decay through the final expiry
                # window: weekly gamma can expand a premium abruptly.
                normal_squareoff = now.hour > AUTO_SQUAREOFF_HOUR or (
                    now.hour == AUTO_SQUAREOFF_HOUR and now.minute >= AUTO_SQUAREOFF_MINUTE
                )
                expiry_squareoff = session_dte_days <= 0.01 and (
                    now.hour > EXPIRY_EXIT_HOUR or
                    (now.hour == EXPIRY_EXIT_HOUR and now.minute >= EXPIRY_EXIT_MINUTE)
                )
                if normal_squareoff or expiry_squareoff:
                    reason_label = "EXPIRY_RISK_EXIT" if expiry_squareoff and not normal_squareoff else "SESSION_END"
                    log_alert(f"🕒 Auto Square-Off ({reason_label}). Liquidating all positions...")
                    self._exit_all_positions(reason=reason_label)
                ...
                # Phase B: Active Trading Management (RUNNING, CHOP_MODE, COOLDOWN)
                    ...
                    for leg in ("CE", "PE"):
                        ...
                        # A short leg is closed by buying at the ask. Use that
                        # executable price for risk checks whenever available;
                        # LTP alone can delay the stop during a fast move.
                        ltp_premium = self._get_exit_premium(
                            self.positions[leg]["strike"], self.positions[leg]["base"]
                        )
                        if ltp_premium <= 0:
                            continue
